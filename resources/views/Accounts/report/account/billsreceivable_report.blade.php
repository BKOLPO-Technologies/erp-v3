@extends('Accounts.layouts.admin', ['pageTitle' => 'Bills Receivable Report'])
                ...
                <div class="col-12">
                    <div class="card card-primary card-outline shadow-lg">
                        ...
                        <div class="card-body mt-3">
                            <!-- Date Filter Form -->
                            <form action="{{ route('accounts.report.bills.receivable') }}" method="GET" class="mb-3">
                                <div class="row justify-content-center">
                                    ...
                                    <div class="col-md-1 mt-3 d-flex align-items-end">
                                        <a href="{{ route('accounts.report.bills.receivable') }}"  class="btn btn-danger w-100">Clear</a>
                                    </div>
                                </div>
                            </form>
                            ...
                            <div class="card-header text-center mb-3">
                                ...
                                <p class="mb-0"><strong>Bills Receivable Report</strong></p>
                                    ...
                                    <div class="col-lg-8 col-md-8 col-sm-12 mx-auto">
                                        ...
                                        <div class="table-responsive">
                                            <table id="example3" class="table-striped table-bordered" style=" width: 100%;">
                                                ...
                                                <tbody>
                                                    @foreach($billsReceivableReports as $key => $report)
                                                        <tr>
                                                            <td class="col-1">{{ $key+1 }}</td>
                                                            <td>{{ $report['invoice_no'] }}</td>
                                                            <td>{{ $report['client_name'] }}</td>
                                                            <td>{{ $report['invoice_date'] }}</td>
                                                            <td class="text-end">{{ bdt() }} {{ number_format($report['grand_total'], 2) }}</td>
                                                            <td class="text-end">{{ bdt() }} {{ number_format($report['paid_amount'], 2) }}</td>
                                                            <td class="text-end">{{ bdt() }} {{ number_format($report['due_amount'], 2) }}</td>
                                                        </tr>
                                                    @endforeach
                                                </tbody>
                                                <tfoot>
                                                    <tr class="fw-bold">
                                                        ...
                                                        <td class="text-end font-weight-bolder">{{ bdt() }} {{ number_format($billsReceivableReports->sum('grand_total'), 2) }}</td>
                                                        <td class="text-end font-weight-bolder">{{ bdt() }} {{ number_format($billsReceivableReports->sum('paid_amount'), 2) }}</td>
                                                        <td class="text-end font-weight-bolder">{{ bdt() }} {{ number_format($billsReceivableReports->sum('due_amount'), 2) }}</td>
                                                    </tr>
                                                </tfoot>
                                            </table>
                                            ...
                                            <div id="amountInWordsPrint" style="margin-top: 10px;">
                                                ...
                                                <strong class="text-uppercase">{{ convertNumberToWords(number_format($billsReceivableReports->sum('grand_total'), 2)) }}</strong>
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        "buttons": [
            ...
            },
            {
                ...
                customize: function (win) {
                    ...
                    $(win.document.head).find('title').text('Bills Receivable Report - {{ get_company()->name ?? "" }}');
                    ...
                    $(win.document.body).prepend(`
                        <div class="text-center mb-2" style="margin-bottom: 10px;">
                            ...
                            <p style="margin: 3px 0; font-weight: bold;">Bills Receivable Report</p>
                    ...
                    $(win.document.body).find('th, td').each(function() {
                        ...
                        if ($(this).text().trim() === 'Invoice No' || $(this).text().trim() === 'Client Name') {
                            $(this).css('width', '15%');
                        }
                        if ($(this).text().trim() === 'Date') {
                            $(this).css('width', '17%');
                        }
                        if ($(this).text().includes('Total') || $(this).text().includes('Paid') || $(this).text().includes('Due')) {
                            $(this).css('width', '17%');
                        }
                    });
